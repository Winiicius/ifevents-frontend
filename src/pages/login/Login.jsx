...
function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      login(response.data.data.accessToken); // token
  ...
  return (
    <div className="h-screen w-full bg-gray-300 p-3">
      <form
        ...
        className="space-y-6 p-5 h-auto w-full md:w-2/4 lg:w-2/4 mx-auto bg-white rounded-md shadow-4xl"
      >
        <FormInput
          label="Login"
          type="text"
        ...
        <button
          type="submit"
          className="bg-green-500 cursor-pointer hover:bg-green-600 transition block w-full p-1.5 rounded-md text-white text-xl"
        >
          Login
        </button>
